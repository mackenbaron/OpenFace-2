...
int KinectWrapper::Update(cv::Mat &bodyImg)
{
	...
	if (mBodyPng.data)
	{
		...
		resize(mBodyPng, mBodyPng, bodyImg.size());
		mBodyPng.copyTo(bodyImg);
		return 1;
	}
	return 0;
}
	...
	const BYTE* pBodyIndexBuffer, int nBodyIndexWidth, int nBodyIndexHeight)
{
	...
	//	if (body.data)
	//	{
	//		//std::unique_lock<std::mutex>(mImgMutex);
	//		mBodyPng = body.clone();
	//	}
	//}
...
cv::Mat KinectWrapper::getBodyImage()
{
	std::unique_lock<std::mutex>(mImgMutex);
	cv::Mat img;
	if(mBodyPng.data)
